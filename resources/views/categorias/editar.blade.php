...
<h1 class="text-2xl font-bold text-center mb-4">Actualizar datos de la categoria {{$categoria->id}}</h1>
			<div class="mb-4">
				<label class="block text-sm font-medium text-gray-700 mb-2">Nombre del producto
				<input type="text" name="nombre" value="{{$categoria->nombre}}" class="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ingrese el nombre">
            </label>    
            </div>
			<div class="mb-4">
        ...
        <br>
        <button class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"><a href="{{route('categoria.principal') }}">Cancelar</a></button>
